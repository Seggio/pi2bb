...
class Pi2BB:
    ...
    def __init__(self, ndim, reward_fn, rollouts_per_step=20, **kwargs):

        # Initial DMPs parameters
        self.ndim = ndim
        self.nbfs = kwargs.get("nbfs", 25)
        dmp_steps = kwargs.get("dmp_steps", 100)
        dmp_duration = kwargs.get("dmp_duration", 1)
        self.dt = dmp_duration / dmp_steps

        # Variance for the perturbation of the DMP parameters
        self.init_variance = self.variance = kwargs.get("variance", 1)

        # Loading DMPs parameters or initializing default ones
        if kwargs.get("load_w", False):
            assert kwargs.get("wdir", None) is not None
            wdir = kwargs.get("wdir")
            y0, g, w = load_weights(wdir)
        else:
            y0, g, w = self.init_params(**kwargs)
        ...
        self.curr_theta = {}
        self.set_theta(self.initial_y0, self.initial_g, self.initial_w)

        # Creating DMPs with initial y0,g,w
        self.dmp = self.create_dmp()
        self.update_dmp()
        ...
        self.sim = None
        self.sim_type = kwargs.get("sim_type", "box2d")

        if self.sim_type == "box2d":
            self.sim = Box2DSimulator(reward_fn, kwargs.get("update_parameters", None))
        elif self.sim_type == "pybullet":
            self.sim = PybulletSim(reward_fn, kwargs.get("update_parameters", None))
